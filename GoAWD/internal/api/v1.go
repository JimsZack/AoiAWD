...
func (v *V1) RegisterRoutes(mux *http.ServeMux, prefix, token string) {
	...
	mux.HandleFunc(prefix+"ping", wrap(v.Ping))
	...
	mux.HandleFunc(prefix+"downloadpwnautoscript", wrap(v.DownloadPWNAutoScript))
...
// maxBinaryUpload caps how much data generatepwnbin accepts (64 MiB of ELF).
const maxBinaryUpload = 64 << 20

// shellQuote wraps s in single quotes so it can be embedded in a shell script
// as a single literal argument. Log fields come from captured traffic and are
// fully attacker controlled, so they must never be interpolated unquoted.
func shellQuote(s string) string {
	return "'" + strings.ReplaceAll(s, "'", `'\''`) + "'"
}

// scriptComment sanitises a value that is placed inside a shell comment or a
// filename: newlines would otherwise let an attacker append arbitrary commands.
func scriptComment(s string) string {
...
func (v *V1) DownloadWebAutoScript(w http.ResponseWriter, r *http.Request) {
	...
	safeID := scriptComment(id)
	script := fmt.Sprintf("#!/bin/bash\n# Auto replay script for web log %s\n# Time: %s\n",
		safeID, scriptComment(formatTimeField(m["time"])))
	script += fmt.Sprintf("curl -sS -X %s %s\n",
		shellQuote(method), shellQuote("http://"+remote+uri))

	w.Header().Set("Content-Type", "text/x-shellscript")
	w.Header().Set("Content-Disposition", "attachment; filename=replay_"+safeID+".sh")
	w.Write([]byte(script))
}
...
func (v *V1) GeneratePWNBin(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(http.MaxBytesReader(w, r.Body, maxBinaryUpload)).Decode(&req); err != nil {
	...
	// Embed the binary so the generated script is self-contained: it drops the
	// ELF into a temporary file and hands it to goawd-guardian.
	encoded := base64.StdEncoding.EncodeToString(elfData)
	wrapped := make([]byte, 0, len(encoded)+len(encoded)/64+256)
	for i := 0; i < len(encoded); i += 76 {
		end := min(len(encoded), i+76)
...
base64 -d >"$TARGET" <<'GOAWD_ELF_EOF'
%s
GOAWD_ELF_EOF
chmod +x "$TARGET"

exec goawd-guardian -binary "$TARGET" -host %s -port %d
`, len(elfData), scriptComment(req.Host), req.Port, wrapped, shellQuote(req.Host), req.Port)
...
func (v *V1) DownloadPWNAutoScript(w http.ResponseWriter, r *http.Request) {
	id := r.URL.Query().Get("id")
	if id == "" {
		writeJSON(w, errResp("id is required"))
		return
	...
	bin, _ := m["bin"].(string)
	safeID := scriptComment(id)
	safeBin := scriptComment(bin)
	script := fmt.Sprintf(`#!/bin/bash
# PWN Auto Replay Script for log %s
# Time: %s
# Binary: %s
# Usage: pipe stdin data to this script

echo "PWN Replay Script Generated by GoAWD"
echo "Log ID: %s"
echo "Binary: %s"
echo ""
echo "To replay, use the stream data from the PWN log."
`, safeID, scriptComment(formatTimeField(m["time"])), safeBin, shellQuote(safeID), shellQuote(safeBin))

	w.Header().Set("Content-Type", "text/x-shellscript")
	w.Header().Set("Content-Disposition", "attachment; filename=pwn_replay_"+safeID+".sh")
	w.Write([]byte(script))
}
